...
fn build_staged(config: &Config, staging: &Path) -> Result<BuildStats, String> {
    ...
    fs::write(staging.join("_headers"), headers())
        .map_err(|error| path_error("write _headers", staging, error))?;
    ...
    fs::write(staging.join(".assetsignore"), format!("{OUTPUT_MARKER}\n.assetsignore\n"))
        .map_err(|error| path_error("write .assetsignore", staging, error))?;
...
mod tests {
    ...
    fn builds_and_merges_dictionary_shards() {
        ...
        let notice = fs::read_to_string(output.join("licenses/wiktionary.txt")).unwrap();
